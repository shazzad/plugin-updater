...
class IntegrationLicenseTest extends TestCase {
	...
	/** @test */
	public function get_license_renewal_url_falls_back_to_details() {
		$integration = $this->create_integration();
		...
		$this->assertSame(
			'https://example.com/renew?license=ABC-123-DEF&email=user@example.com',
			$integration->get_license_renewal_url( $details )
		);
	}

	/** @test */
	public function get_license_renewal_url_prefers_license_over_details() {
		$integration = $this->create_integration();
		...
		$details = [
			'renewal_url' => 'https://details.example.com/renew',
		];
